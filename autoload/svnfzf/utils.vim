" autoload/svnfzf/utils.vim

function! svnfzf#utils#is_svn_repo() abort
  let l:output = system('svn info 2>&1')
  return v:shell_error == 0
endfunction

function! svnfzf#utils#run(cmd) abort
  let l:output = system(a:cmd)
  if v:shell_error != 0
    echohl ErrorMsg
    echom '[svnfzf] ' . substitute(l:output, '\n$', '', '')
    echohl None
    return ''
  endif
  return l:output
...
function! svnfzf#utils#parse_status(raw) abort
  let l:entries = []
...
function! svnfzf#utils#group_status(entries) abort
...
function! svnfzf#utils#svn_wc_prefix() abort
...
function! svnfzf#utils#parse_log(raw) abort
  let l:entries = []
...
function! svnfzf#utils#format_log(entries) abort
  let l:lines = []
  for l:e in a:entries
